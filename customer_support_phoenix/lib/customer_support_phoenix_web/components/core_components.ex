defmodule CustomerSupportPhoenixWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: CustomerSupportPhoenixWeb.Gettext
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(CustomerSupportPhoenixWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(CustomerSupportPhoenixWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  @doc """
  Renders a simple SVG spinner. Useful for indicating loading states.
  ...
  """
  attr :rest, :global, doc: "the arbitrary HTML attributes to add to the svg tag"
  def spinner(assigns) do
    ~H"""
    <svg
      class={[
        "animate-spin h-5 w-5 text-indigo-500",
        assigns[:class] || ""
      ]}
